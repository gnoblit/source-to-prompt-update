...
export function getTauriCliLockPath(repositoryRoot) {
  return path.resolve(repositoryRoot, 'apps/tauri/.tauri-cli.lock');
}

export async function acquireTauriCliLock(repositoryRoot, options = {}) {
  if (!repositoryRoot) {
    throw new TypeError('acquireTauriCliLock requires a repository root');
  }
  ...
  const lockPath = getTauriCliLockPath(repositoryRoot);

  const lockPayload = {
    pid,
    args,
    command: `tauri ${args.join(' ')}`.trim(),
